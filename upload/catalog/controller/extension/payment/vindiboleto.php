<?php
class ControllerExtensionPaymentYapayb extends Controller {
	public function index() {
		...
		return $this->load->view('extension/payment/yapayb', $data);
	}

	public function confirm() {
	    $json = array(); 
		if ($this->session->data['payment_method']['code'] == 'yapayb') {
        include_once('yapaybcode.php');
	    }
	...
	public function getBoleto($json_convert) {
	    
	if ($this->config->get('payment_yapayb_type') == 0) {
	...
	public function callback() {
	    ...
	    if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if ($order_info && $this->request->post['token_transaction'] && $this->request->post['transaction']['transaction_id'] && $order_info['payment_code'] == 'yapayb') {
			    
		        $order_status_ids = $order_info['order_status_id'];
				$order_status_id = $this->config->get('payment_yapayb_order_status_id');
				...
				switch($this->request->post['transaction']['status_id']) {
					case '4':
						...
						break;
					case '6':
						$order_status_id = $this->config->get('payment_yapayb_order_status_id2');
						break;
					case '7':
						$order_status_id = $this->config->get('payment_yapayb_order_status_id1');
						break;
					case '24':
						$order_status_id = $this->config->get('payment_yapayb_order_status_id3');
						break;
					case '87':
						$order_status_id = $this->config->get('payment_yapayb_order_status_id');
						break;
	        ...
	        $this->log->write('ERRO no Retorno: Yapay Boleto - IP '. $this->request->server['REMOTE_ADDR']);
	    }
